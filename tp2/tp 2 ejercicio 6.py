...
def procesar_fila(db, fila):
    ...
    key = f"especialidad= {fila['nombre_especialidad']}:{fila['nombre_tipo_especialidad']}"
    torneo = fila['nombre_torneo']
    intento = fila['intento']
    nombre_deportista = fila['nombre_deportista']
    valor = f"{torneo}:{intento}:{nombre_deportista}"
...
def generar_reporte(db):
    ...
    for clave in claves:
            ...
            for i, (valor, score) in enumerate(top_3):
                torneo, intento, nombre_deportista = valor.split(':')
                posicion = i + 1
                linea = (
                    f"{especialidad} - Posición {posicion}: "
                    f"Deportista: {nombre_deportista}, "
                    f"Marca: {score}, "
                    f"Torneo: {torneo}, Intento: {intento}"
                )
