...
nombre_archivo_resultado_ejercicio= 'C:/Users/Kalou/OneDrive/Escritorio/BasesDatosNoSQL/TP3/tp3_ej06.txt'
...
def inicializar(conn):
    ...
    cassandra_session.execute("""
                              CREATE KEYSPACE IF NOT EXISTS set_keyspace66
                            WITH replication = {'class': 'SimpleStrategy', 'replication_factor': 1}
                                """)
    cassandra_session.set_keyspace("set_keyspace66")
    cassandra_session.execute("""
                                CREATE TABLE IF NOT EXISTS torneo_especialidad_tipo_especialidad(
                                    id_especialidad INT,
                                    especialidad TEXT,
                                    ...
                                    intento INT,
                                    marca FLOAT,
                                    PRIMARY KEY ((id_especialidad, id_torneo), marca)
                                )
...
def procesar_fila(db, fila):
    ...
    query = """
    INSERT INTO torneo_especialidad_tipo_especialidad (id_especialidad, nombre_deportista, especialidad, nombre_tipo_especialidad, id_torneo, torneo, intento, marca)
    VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
    """
    db.execute(query, (id_especialidad,nombre_deportista, especialidad, nombre_tipo_especialidad, id_torneo ,torneo, intento, marca))
        
    query3 = """
    INSERT INTO especialidades_unicas2 (id_especialidad, especialidad, nombre_tipo_especialidad, id_torneo )
    VALUES (%s, %s, %s, %s)
    """
    db.execute(query3, (id_especialidad, especialidad, nombre_tipo_especialidad, id_torneo ))
...
def generar_reporte(db):
    ...
    with open(nombre_archivo_resultado_ejercicio, "w", encoding="utf-8") as archivo:
        ...
        SELECT id_especialidad , nombre_tipo_especialidad, id_torneo FROM especialidades_unicas2
        """
        torneos = db.execute(query_especialidades)

        for torneo in torneos:
            id_especialidad = torneo.id_especialidad
            tipo_especialidad = torneo.nombre_tipo_especialidad
            ...
            query_top3 = f"""
                ...
                FROM torneo_especialidad_tipo_especialidad 
                WHERE id_especialidad = %s and id_torneo = %s
                ORDER BY marca {tipo_ordenamiento} 
                LIMIT 3
            """
            
            top3_rows = db.execute(query_top3, (id_especialidad,id_torneo,))

            # Escribimos las filas del podio en el archivo
            for row in top3_rows:
                linea = (f"{row.torneo},{row.nombre_tipo_especialidad}, {row.especialidad}, {row.nombre_deportista}, {row.marca}, {row.intento}")
