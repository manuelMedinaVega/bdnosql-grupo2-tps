...
def procesar_fila(database, fila):
    database.add((fila['id_deportista'], fila['nombre_deportista']))
...
def generar_reporte(database):
    ...
    for fila in database:
        id_deportista, nombre_deportista = fila
        id_deportista = int(id_deportista)  # Convertir a entero si es necesario
        print(id_deportista)
        if id_deportista in [10, 20, 30]:
                    grabar_linea(archivo, f"{id_deportista} - {nombre_deportista}")
